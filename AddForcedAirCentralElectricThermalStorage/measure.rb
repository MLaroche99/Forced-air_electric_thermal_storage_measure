class AddForcedAirCentralElectricThermalStorage < OpenStudio::Measure::ModelMeasure
  ...
  def modeler_description
    return 'Code modified by M. Mathieu Laroche, developed by M. Houssem Eddine Younes, M.Sc. Supervised by Mme. Katherine DAvignon, Ph.D  & M. François Laurencelle, Ph.D. Funded by Mitacs Inc and Hydro-Québec.'
  end
  ...
  def arguments(model)
	...
	pheat_max.setDisplayName('Enter device maximum discharge rate (Pheat,test)[kW]:')
	pheat_max.setDefaultValue(15)
	args << pheat_max
	...
	brick_temp_deadband.setDisplayName('Enter the brick core temperature deadband [°C]:')
 ...
 #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
  ...
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	ELSEIF #{storage_electric_pwr.name} > 0
		SET #{brick_setpoint_error.name} = #{brick_target_core_temp.name} + #{brick_deadband.name} - #{brick_init_temp.name}
	ELSE
		SET #{brick_setpoint_error.name} = #{brick_target_core_temp.name} - #{brick_deadband.name} - #{brick_init_temp.name}
	ENDIF
